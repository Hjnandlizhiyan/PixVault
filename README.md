...
- **双入口批量导入**：可通过系统相册直观选图，也可选择“导入原图并保留位置”，自动拷贝到应用私有目录并读取尺寸、体积、MIME、EXIF 位置等元数据；
...
- **设置页**：按外观、隐私、AI、存储、帮助分组；帮助中可跳转 GitHub、作者 Bilibili 主页和个人网站，二维码只在反馈入口弹窗中显示。
...
**权限说明**：应用不申请网络或全盘存储权限，图片通过系统 Photo Picker 选择。为了让“照片地图”读取 Android 10 及以上系统默认隐藏的 EXIF GPS，导入时会按需请求 `ACCESS_MEDIA_LOCATION`；拒绝后仍可正常导入，只是位置可能无法显示。应用自身全程离线。
...
首页点击**导入**按钮后可选择两个入口：**从相册导入**会打开直观的系统相册选择器；**导入原图并保留位置**会打开原始文件选择器，适合需要在照片地图中保留 EXIF 拍摄位置的照片。确认后应用会把图片拷贝到私有目录并自动计算图像向量，进度以"计算特征 n/N"显示。若在选择时同时指定标签，则这些图片会立刻归入该标签。
...
- AI：进入相似照片整理；
- 存储：进入照片地图与回收站；
- 帮助：访问 GitHub 项目、作者 Bilibili 主页、个人网站，重新播放新手指引、查看角色表情，以及按需打开反馈群二维码。
