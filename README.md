...
- 接入横向时间线场景插画、六态角色表情、角色加载动画与统一矢量图标组；角色分别用于搜索、加载、私密和整理状态。
...
- **私密空间**：普通图库、收藏、标签与搜索自动排除私密照片；使用系统认证解锁，私密页面启用防截图；
...
- **设置页**：按外观、隐私、AI、存储、帮助分组；二维码只在反馈入口弹窗中显示。
...
| TimelineScreen.kt / TimelineGallery.kt | 时间线场景、日期分组网格与吸顶月份栏 |
| PrivateSpaceScreen.kt | 系统认证、私密图库与防截图 |
...
- AI：进入相似照片整理；
- 存储：进入照片地图与回收站；
- 帮助：重新播放新手指引、查看角色表情，以及按需打开反馈群二维码。
